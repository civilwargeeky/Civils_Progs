...
function menu(title, description, menuList, isNumbered, titleAlign, textAlign, prefixCharacter, suffixCharacter)
...
if not (title and menuList) then error("Requires title and menu list",2) end
if not type(menuList) == "table" and #menuList >= 1 then error("Menu list must be a table with values",2) end
...
while true do
  ...
  if currIndex >= bottom and bottom < #menuList then --If at bottom scroll down. Change to > instead of >= to only do on bottom line. Same for above
    scroll = scroll + 1
    top, bottom = top + 1, bottom + 1
  end
  term.clear()
  ...
  for i = 1, math.min(y - titleLines,#menuList) do --The min because may be fewer table entries than the screen is big
    ...
    if not (#(prefix..menuList[i+scroll]..suffix) <= x) then term.clear(); term.setCursorPos(1,1); error("Menu item "..tostring(i+scroll).." is longer than one line. Cannot Print",2) end
    output(prefix..menuList[i+scroll]..suffix, i + titleLines, textAlign)
  end
  _, key = os.pullEvent("key")
  if key == 200 and currIndex > 1 then
    currIndex = currIndex - 1
  end
  if key == 208 and currIndex < #menuList then
    currIndex = currIndex + 1
  end
  if key == 28 then
    return currIndex
...
a = menu(titleize("Fruit and Letter picker"), "What will you pick today?", list, false, nil, "left", "(=^.^=)","")
